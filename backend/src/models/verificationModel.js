...
export const createCode = async (userId, code, type = "verification") => {
  await pool.query(
    `UPDATE email_verification_codes SET used = true WHERE user_id = $1 AND used = false AND type = $2`,
    [userId, type]
  );
...
export const findValidCode = async (userId, code, type = "verification") => {
  const result = await pool.query(
    `SELECT * FROM email_verification_codes
     WHERE user_id = $1 AND code = $2 AND used = false AND expires_at > NOW() AND type = $3
